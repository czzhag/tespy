...
def main():
	...
	if small:
		out_fig = out_path_main+'spec_%d/'%highp
		if not os.path.isdir(out_fig):
			os.makedirs(out_fig)
	else:
		out_fig = out_path_main+'spec_large_%d/'%highp
		if not os.path.isdir(out_fig):
			os.makedirs(out_fig)
	...
	if os.path.exists(fnpickle):
		...
		bc_array=dexist[1]
		eff_array=dexist[2]
		detcol_array=dexist[3]
		detrow_array=dexist[4]
		detpol_array=dexist[5]
	...
	#for col in [8]:
		...
		#for row in [15,16,17,18]:
			...
			if not savefigdetcoord:
				...
				figname = 'spec_im'+str(int(im))+'_detrow'+str(int(detrow))+'_detcol'+str(int(detcol))+'_'+detpol
